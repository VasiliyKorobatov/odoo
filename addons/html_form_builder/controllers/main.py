...
class HtmlFormController(http.Controller):
    def _html_action_custom_server_action(self, submit_action, history_data):
        ...
        request.session.model('ir.actions.server').run(submit_action.custom_server_action.id,
                                                       {'active_id': form_record.id,
                                                        'active_model': history_data.html_id.model_id.model})
    ...
    def form_field_config_input_group(self, **kw):
        ...
        for field in request.env['ir.model.fields'].search(
            ...
            if (
                            field.ttype == 'char' or field.ttype == 'integer' or field.ttype == 'selection' or field.ttype == 'binary'):
                field_options_html += "<input type=\"checkbox\" name=\"input_group_fields\" value=\"" + str(
                    field.id) + "\"/> " + str(field.field_description) + " (" + str(field.ttype) + ")<br/>\n"
    ...
    def form_field_config_general(self, **kw):
        ...
        for my_field in request.env['ir.model.fields'].search(
                [('ttype', 'in', data_types_list), ('model_id.model', '=', values['form_model']),
                 ('name', '!=', 'display_name')]):
            field_options_html += "<option value=\"" + str(my_field.id) + "\">" + my_field.field_description.encode(
                "utf-8") + " (" + my_field.name.encode("utf-8") + "/" + str(my_field.ttype) + ")" + "</option>\n"
    ...
    def _generate_html_date_picker(self, field):
        ...
        html_output += "<div class=\"hff hff_date_picker form-group\" data-form-type=\"" + field.field_type.html_type + "\" data-field-id=\"" + str(
            field.id) + "\">\n"
        html_output += "  <label class=\"control-label\" for=\"" + field.html_name.encode(
            "utf-8") + "\">" + field.field_label + "</label>\n"
        html_output += "  <div class=\"input-group date\">\n"
        html_output += "    <input type=\"text\" class=\"form-control\" id=\"" + field.html_name.encode(
            "utf-8") + "\" name=\"" + field.html_name.encode("utf-8") + "\""
    ...
    def _generate_html_datetime_picker(self, field):
        ...
        html_output += "<div class=\"hff hff_datetime_picker form-group\" data-form-type=\"" + field.field_type.html_type + "\" data-field-id=\"" + str(
            field.id) + "\">\n"
        html_output += "  <label class=\"control-label\" for=\"" + field.html_name.encode(
            "utf-8") + "\">" + field.field_label + "</label>\n"
        html_output += "  <div class=\"input-group date\">\n"
    ...
    def _generate_html_textbox(self, field):
        ...
        html_output = ""
        html_output += "<div class=\"hff hff_textbox form-group\" data-form-type=\"" + field.field_type.html_type + "\" data-field-id=\"" + str(
            field.id) + "\">\n"
        html_output += "  <label class=\"control-label\" for=\"" + field.html_name.encode(
            "utf-8") + "\">" + field.field_label + "</label>\n"
    ...
    def _generate_html_textarea(self, field):
        ...
        html_output = ""
        html_output += "<div class=\"hff hff_textarea form-group\" data-form-type=\"" + field.field_type.html_type + "\" data-field-id=\"" + str(
            field.id) + "\">\n"
        html_output += "  <label class=\"control-label\" for=\"" + field.html_name.encode(
            "utf-8") + "\">" + field.field_label + "</label>\n"
    ...
    def _generate_html_file_select(self, field):
        ...
        html_output += "<div class=\"hff hff_binary form-group\" data-form-type=\"" + field.field_type.html_type + "\" data-field-id=\"" + str(
            field.id) + "\">\n"
        html_output += "  <label class=\"control-label\" for=\"" + field.html_name.encode(
            "utf-8") + "\">" + field.field_label + "</label>\n"
    ...
    def _generate_html_input_group(self, field):
        ...
        html_output += "      $( \"#" + field.html_name.encode(
            "utf-8") + "_container\" ).append( '<div class=\"row form-group\">' + $( \"#" + field.html_name.encode(
            "utf-8") + "_placeholder\").html() + '</div>');\n"
        html_output += "    });\n"
        html_output += "  });\n"
        html_output += "</script>\n"
        ...
        html_output += "<div class=\"hff hff_input_group form-group\" data-sub-field-number=\"" + str(
            sub_field_number) + "\" data-html-name=\"" + field.html_name.encode(
            "utf-8") + "\" data-form-type=\"" + field.field_type.html_type + "\" data-field-id=\"" + str(
            field.id) + "\">\n"
        html_output += "  <label class=\"control-label\" for=\"" + field.html_name.encode(
            "utf-8") + "\">" + field.field_label + "</label><br/>\n"
        ...
        for sub_field in field.setting_input_group_sub_fields:
            if sub_field.ttype == "char" or sub_field.ttype == "integer":
                html_output += "      <div class=\"col-md-" + str(
                    column_width) + "\"><input type=\"text\" class=\"form-control\" data-sub-field-name=\"" + str(
                    sub_field.name) + "\" placeholder=\"" + str(sub_field.field_description) + "\"/></div>\n"
            elif sub_field.ttype == "selection":
                html_output += "      <div class=\"col-md-" + str(column_width) + "\">\n"
                html_output += "<select class=\"form-control\" data-sub-field-name=\"" + str(
                    sub_field.name) + "\" placeholder=\"" + str(sub_field.field_description) + "\">\n"
                ...
                for selection_value, selection_label in selection_list.items():
                    html_output += "<option value=\"" + selection_value.encode(
                        "utf-8") + "\">" + selection_label + "</option>\n"

                html_output += "</select>\n"
                html_output += "</div>\n"
            elif sub_field.ttype == "binary":
                html_output += "      <div class=\"col-md-" + str(
                    column_width) + "\"><input type=\"file\" data-sub-field-name=\"" + str(
                    sub_field.name) + "\" placeholder=\"" + str(sub_field.field_description) + "\"/></div>\n"
        ...
        html_output += "    <button class=\"btn btn-primary btn-md row pull-right\" id=\"" + field.html_name.encode(
            "utf-8") + "_add\">Add (+)</button>\n"
        html_output += "  </div>\n"
    ...
    def _generate_html_radio_group_selection(self, field):
        ...
        html_output = ""
        html_output += "<div class=\"hff hff_radio_group form-group\" data-form-type=\"" + field.field_type.html_type + "\" data-field-id=\"" + str(
            field.id) + "\">\n"
        ...
        for selection_value, selection_label in selection_list.items():
            ...
            html_output += "    <label><input type=\"radio\" name=\"" + field.html_name.encode(
                "utf-8") + "\" value=\"" + selection_value.encode("utf-8") + "\"/>" + selection_label.encode(
                "utf-8") + "</label>\n"
            html_output += "  </div>\n"
    ...
    def _generate_html_checkbox_group(self, field):
        ...
        html_output = ""
        html_output += "<div class=\"hff hff_checkbox_group\" data-form-type=\"" + field.field_type.html_type + "\" data-field-id=\"" + str(
            field.id) + "\">\n"
        ...
        for my_record in request.env[field.field_id.relation].search([('name', '!=', '')]):
            ...
            html_output += "    <label><input type=\"checkbox\" value=\"" + str(
                my_record.id) + "\" name=\"" + field.html_name.encode("utf-8") + "\"/>" + my_record.name.encode(
                "utf-8") + "</label>\n"
            html_output += "  </div>\n"
    ...
    def _generate_html_checkbox_boolean(self, field):
        ...
        html_output = ""
        html_output += "<div class=\"hff hff_checkbox checkbox\" data-form-type=\"" + field.field_type.html_type + "\" data-field-id=\"" + str(
            field.id) + "\">\n"
        html_output += "  <label><input type=\"checkbox\" name=\"" + field.html_name.encode(
            "utf-8") + "\"/>" + field.field_label + "</label>\n"
        html_output += "</div>\n"
    ...
    def _generate_html_dropbox(self, field):
        ...
        html_output += "<div class=\"hff hff_dropbox form-group\" data-form-type=\"" + field.field_type.html_type + "\" data-field-id=\"" + str(
            field.id) + "\">\n"
        html_output += "  <label class=\"control-label\" for=\"" + field.html_name.encode(
            "utf-8") + "\">" + field.field_label
        html_output += "</label>\n"
        ...
        if field.field_id.ttype == "selection":
            ...
            for selection_value, selection_label in selection_list.items():
                html_output += "    <option value=\"" + selection_value.encode(
                    "utf-8") + "\">" + selection_label.encode("utf-8") + "</option>\n"
    ...
    def load_form(self, **kw):
        ...
        if html_form.captcha:
            form_string += "<div class=\"html_form_captcha col-md-12 form-group\" data-captcha-id=\"" + str(
                html_form.captcha.id) + "\">\n"
            form_string += "  <div class=\"g-recaptcha\" data-sitekey=\"" + str(
                html_form.captcha_client_key) + "\"></div>\n"
            form_string += "</div>\n"
    ...
    def form_add_field(self, **kw):
        ...
        if len(request.env['html.form.field'].search(
                [('html_id', '=', int(values['form_id'])), ('field_id', '=', field_id.id)])) > 0:
        ...
        if 'sub_fields' in values: insert_values['setting_input_group_sub_fields'] = [
    ...
    def process_form(self, kwargs):
        ...
        if form_error:
            ...
            new_history = http.request.env['html.form.history'].sudo().create(
                {'ref_url': ref_url, 'html_id': entity_form.id})

            for insert_field in insert_data_dict:
                new_history.insert_data.sudo().create({'html_id': new_history.id, 'field_id': insert_field['field_id'],
            ...
            for df in entity_form.defaults_values:
                if df.field_id.ttype == "many2many":
                    secure_values[df.field_id.name] = [
                        (4, request.env[df.field_id.relation].search([('name', '=', df.default_value)])[0].id)]
                else:
                ...
                new_history.insert_data.sudo().create(
                    {'html_id': new_history.id, 'field_id': df.field_id.id, 'insert_value': df.default_value})
