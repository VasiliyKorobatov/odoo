...
class PriceRule(models.Model):
    ...
    def _get_name(self):
        for rule in self:
            ...
            if rule.list_base_price and not rule.list_price:
                ...
                name = '%s %s за %s' % (name, rule.list_price, rule.variable_factor)
            else:
                name = '%s фиксированная цена %s + %s за %s дополнительно' % (name, rule.list_base_price, rule.list_price, rule.variable_factor)
            rule.name = name
    ...
    variable = fields.Selection([('вес', 'Weight'), ('объем', 'Volume'), ('ВО', 'Weight * Volume'), ('цена', 'Price'), ('кол-во', 'Quantity')], 'Variable', required=True, default='вес')
    operator = fields.Selection([('==', '='), ('<=', '<='), ('<', '<'), ('>=', '>='), ('>', '>')], 'Operator', required=True, default='<=')
    max_value = fields.Float('Maximum Value', required=True)
    variable_factor = fields.Selection([('вес', 'Weight'), ('объем', 'Volume'), ('ВО', 'Weight * Volume'), ('цена', 'Price'), ('кол-во', 'Quantity')], 'Variable Factor', required=True, default='вес')
